...
The packages can structure supplied information and expose missing evidence. They cannot independently verify unavailable source data, make an organization's decision, or ensure a proposal succeeds. Planning's external comparison is conditional on relevant sources being supplied or researched; reporting's license remains `Proprietary`; the repository has no unified license or public remote.
...
These results show format and method consistency in these prompts, not a general score gain. The repository currently contains 24 eval prompts across the four skills. Six selected scenarios have paired baseline/skill spot checks; five additional pressure scenarios remain candidate regression cases without a matched behavior run. The updated planning competitor-reference and weekly metric-review scenarios were rerun after their method references changed; both retained a 10/10 rubric score, matching their strong baseline/old-skill answers. See each skill's `evals/evals.json`; do not describe this small qualitative set as broad effectiveness evidence.
