...
The packages can structure supplied information and expose missing evidence. They cannot independently verify unavailable source data, make an organization's decision, or ensure a proposal succeeds. Planning's external comparison is conditional on relevant sources being supplied or researched; reporting's license remains `Proprietary`; the GitHub remote exists at `Liyuk/engineering-planning-review-skills` but is private, and the repository has no unified license.
